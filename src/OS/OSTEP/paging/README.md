...
真實的位址空間肯定大得多，通常 32 bits 的系統有 4GB 的位址空間，更不用說現在還有 64 bits 的系統。 在本書中，我們常常舉些小例子，讓大家更容易理解

物理記憶體，如圖 18.2 所示，是一組固定元素長度的陣列。 在這個例子中，有 8 個 page frame（由 128 bytes 物理記憶體構成，很小）。 從圖中可以看出，虛擬位址空間的 virtual page 放在物理記憶體的不同位置，你還可以發現 OS 自己也用了一些物理記憶體

可以看到，與我們以前的方法相比，paging 有許多優點。 最大的優點是靈活性，通過 paging，OS 能夠高效地抽象化位址空間，不需理會 process 如何使用位址空間，例如我們不用管 heap 和 stack 的增長方向，以及它們是如何被使用的
...
為了記錄每個 virtual page 放在物理記憶體中的位置，OS 通常會為每個 process 保存一個資料結構，稱為 page table。 page table 的主要作用是為每個 virtual page 保存其位址轉換的關係（address translation），從而讓我們知道每個 virtual page 在物理記憶體中的位置
...
現在來看一個位址轉換的例子。 假設擁有這個小位址空間（64 bytes）的 process 正在訪問記憶體：

```asm
movl <virtual address>, %eax
```

這邊我們關注從位址 `<virtual address>` 到暫存器 `eax` 的顯式載入（忽略 instruction fetch）。 為了轉換該虛擬位址，我們必須先將它分成兩個部分：
...
當 process 生成虛擬位址時，OS 和硬體必須合作，將它轉換為有意義的物理位址。 假設載入虛擬位址為 `21`：
...
通過 VPN，我們現在可以查找 page table，找到 virtual page `1` 所在的 page frame。 在上面的 page table 中，physical frame number（PFN）是 7（二進制 `111`）。 因此，我們可以通過用 PFN 替換 VPN 來轉換此虛擬位址，然後將載入發送給物理記憶體：
...
假設該系統的虛擬位址分成 20 位的 VPN 和 12 位的偏移量，一個 20 位的 VPN 意味著 有 $2^{20}$ 個 virtual page，因此 OS 必須為每個 process 管理 $2^{20}$ 個位址轉換（大約一百萬）

> 有 $2^{20}$ 個 virtual page，每個 virtual page 的大小為 $2^{12}$ bytes，則整個位址空間有 $2^{20} \times 2^{12}$ bytes 這麼大，符合我們的假設

假設每個 page table 的元素（被稱為 page table entry，簡記為 PTE）需要 4 bytes，以保存物理位址轉換和任何其他有用的東西，則單個 page table 就會需要 4MB 記憶體，這超大

現在想象一下有 100 個 process 在運行：這代表 OS 需要花費 400MB 的記憶體，僅僅是為了做位址轉換。 即使是現在，機器擁有 GB 級別的記憶體，這也是占了很大一部分，更不用提 64-bit 系統的 page table 了，又會再變更大
...
在這個例子中，我們假設由硬體執行位址轉換。 要獲取所需資料，系統必須先將虛擬位址（`21`）轉換為正確的物理位址（`117`）。 因此，在從位址 `117` 獲取資料之前，系統必須先從 process 的 page table 中找到對應的 PTE，執行轉換得到 PFN，然後從物理記憶體中載入資料
...
例如，使用虛擬位址 `21`（`010101`），`VPN MASK` 將此值轉換為 `010000`，`SHIFT` 將它變成 `01`，也就是第二個 virtual page（VPN 為 1），正是我們期望的值
...
你可以發現，對於每個記憶體操作（無論是取指令，還是顯式載入/儲存），paging 都需要我們執行一個額外的記憶體操作，以從 page table 中獲取位址轉換的關係，而額外記憶體操作的開銷很大，在這種情況下，可能會使 process 減慢兩倍或更多
...
::: info  
現代 OS 的記憶體管理子系統中最重要的資料結構之一就是 page table。 通常 page table 儲存虛擬 ⬌ 物理位址轉換的關係（virtual-to-physical address translation），從而讓系統知道位址空間的每個 page 實際在物理記憶體中的哪個位置

由於每個位址空間都需要這種轉換，因此一般來說，系統中每個 process 都有一個 page table
...
為了理解這個指令序列（在虛擬層和物理層）所訪問的記憶體，我們必須假設虛擬記憶體中程式碼片段和陣列的位置，以及 page table 的內容和位置。 對於這個例子，假設一個大小為 64KB 的虛擬位址空間，page 大小為 1KB

我們現在需要知道 page table 的內容，以及它在物理記憶體中的位置。 假設有一個 linear page table，位於物理位址 1KB（`1024`）。 至於其內容，我們只關心於這個例子中映射的幾個 virtual page

由於 page 大小為 1KB，虛擬位址 `1024` 位於虛擬位址空間的第二 page（VPN = 1，VPN 從 0 開始），例子內的陣列大小為 4000 bytes（1000 個整數），假設它位於虛擬位址 `40000` 到 `44000`（不包括最後一個 bytes）。 其 virtual page 的十進制範圍是 `VPN = 39` 至 `VPN = 42`
...
此外，程式中還有一個明確的記憶體存取：`mov` 指令； 這會另外先多訪問一次 page table，以將陣列的虛擬位址轉換為正確的物理位址），再來才是對陣列本身進行存取
...
- 每次迴圈總共有 10 次記憶體存取，包含四次指令擷取（instruction fetch）、一次對記憶體的明確寫入，以及五次 page table 的存取，用來轉換這四次擷取和一次寫入的虛擬位址

> 這裡假設每條指令的大小都是 4 bytes，實際上，x86 指令是可變大小的

### 18.6 小結

我們已經介紹了 paging 的概念，作為虛擬化記憶體挑戰的解決方案，相比於先前的方法（segmentation）有許多優點

- 它不會導致外部碎片，因為 paging 本身的設計就是將記憶體劃分為固定大小的單位
- 它具有高度彈性，允許虛擬位址空間的稀疏使用
