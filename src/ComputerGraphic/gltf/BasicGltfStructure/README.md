...
- 骨架（skin）定義了物件的幾何形狀如何根據骨骼姿勢進行變形
- 相機（camera）則描述了渲染時的視角設定

## The JSON structure

場景中的各種物件會以陣列（array）的方式儲存在 JSON 檔案中。 可以透過物件在陣列中的索引值（index）來存取對應的物件：
...
這些索引同時也用來定義物件之間的關係。 上面的例子定義了多個網格（mesh），而節點（node）可以透過引用某個 mesh 的索引，來指定這個節點附加了哪一個 mesh：
...
- [`scene`](https://www.khronos.org/registry/glTF/specs/2.0/glTF-2.0.html#reference-scene)：描述 glTF 中儲存的場景內容的起點。 它會參考一組 node，這些 node 構成了場景圖（scene graph）
- [`node`](https://www.khronos.org/registry/glTF/specs/2.0/glTF-2.0.html#reference-node)：場景圖中的單一節點。 每個節點可以包含一個變換（例如旋轉或平移），也可以指向其他（子）節點。 此外，節點可以掛載一個 mesh 或 camera，或者掛上一個描述網格變形的 skin
...
- [`accessor`](https://www.khronos.org/registry/glTF/specs/2.0/glTF-2.0.html#reference-accessor)：作為各種資料的抽象來源。 它被 mesh、skin、animation 使用，負責提供幾何資料、蒙皮參數與隨時間變化的動畫數值。 accessor 會參考 [`bufferView`](https://www.khronos.org/registry/glTF/specs/2.0/glTF-2.0.html#reference-bufferview)，而 bufferView 又是 [`buffer`](https://www.khronos.org/registry/glTF/specs/2.0/glTF-2.0.html#reference-buffer) 中某一段實際的原始二進位資料
...
讀取與處理 glTF asset 的第一步是解析 JSON 結構。 當結構解析完成後，[`buffer`](https://www.khronos.org/registry/glTF/specs/2.0/glTF-2.0.html#reference-buffer) 和 [`image`](https://www.khronos.org/registry/glTF/specs/2.0/glTF-2.0.html#reference-image) 物件會分別出現在頂層的 `buffers` 和 `images` 陣列中。 每個這樣的物件可能會指向一塊二進位資料區塊。 為了進一步處理，這些資料會被讀入記憶體中。 通常，資料會以陣列的形式儲存，並且可以透過與對應的 `buffer` 或 `image` 物件相同的索引值來查詢
